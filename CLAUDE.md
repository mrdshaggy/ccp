...
## Deployment
- **Production**: Vercel at `https://tsinoshchuk.vercel.app` — GitHub repo `mrdshaggy/tsinoshchuk`, auto-deploys on push to `main`
- **Dev**: `npm run dev` (Vite dev server with 4 proxies)
- `.github/workflows/deploy.yml` exists but is no longer used (superseded by Vercel)
- `vite` and `@vitejs/plugin-react` are in `dependencies` (not `devDependencies`) — required because Vercel sets `NODE_ENV=production` and would otherwise skip them during build
...
├── data/
│   └── atbStores.json             # 1235 ATB stores — static snapshot from OpenStreetMap (June 2026)
├── utils/
...
vite.config.js                     # base: '/', 4 dev proxies: /silpo-api, /silpo-branches, /metro-api, /metro-www
vercel.json                        # installCommand + 4 rewrites (same 4 APIs, server-side CORS proxy)
```
...
| **АТБ** | Static JSON — `src/data/atbStores.json` (1235 stores, fetched June 2026) | Live — `api.multisearch.io` | Products: `id=11280`, `key=63a6d0a760fd2d0562c4061b78e64754`, CORS-enabled (no proxy needed). National pricing (not store-specific). Weight parsed from title. Store list is a snapshot — refresh periodically by re-running the Overpass query locally. |
...
| File | Role |
|------|------|
| `vite.config.js` | `base: '/'`. 4 dev proxies: `/silpo-api` → `api.catalog.ecom.silpo.ua`, `/silpo-branches` → `sf-ecom-api.silpo.ua`, `/metro-api` → `shop.metro.ua`, `/metro-www` → `www.metro.ua`. No ATB proxy needed (stores are static, products hit CORS-enabled API directly). |
| `vercel.json` | `installCommand: "npm install --include=dev"` + 4 rewrites mirroring the 4 Vite proxies. No ATB rewrite. |
| `src/data/atbStores.json` | Static array of `{id, title, address, lat, lon}` objects. Generated by running the Overpass query `["brand:wikidata"="Q4054103"]` bbox Ukraine, then mapping to the app format. Overpass-api.de blocks Vercel/AWS IPs, so this cannot be fetched at runtime on Vercel — must be a static file. |
| `src/services/api.js` | All fetch logic. `getStores(hub, userPos?)` and `searchProducts(store, query, chainKey)` are the public exports. ATB stores: synchronous import of `atbStores.json`, mapped to `{id, title, address, coordinates:{latitude,longitude}}`. ATB products: `extractAtbWeight()` parses weight string from product title (кг→кг, г→г, мл→мл, л→л). |
...
## ATB store snapshot refresh

When ATB stores need updating, run locally (Vercel IPs are blocked by overpass-api.de):

```bash
curl -s -X POST \
  -H "Content-Type: application/x-www-form-urlencoded" \
  --data-urlencode 'data=[out:json][timeout:60];(node["brand:wikidata"="Q4054103"](44,22,52.5,40.5);way["brand:wikidata"="Q4054103"](44,22,52.5,40.5););out center;' \
  https://overpass-api.de/api/interpreter > /tmp/atb_raw.json
```

Then process with Node and replace `src/data/atbStores.json`.

---

## Known limitations / pending tasks

- **ATB stores are a snapshot** — `src/data/atbStores.json` was fetched June 2026; new stores won't appear until manually refreshed
...
- **Compare modal matching** is heuristic — may not match across chains for products with no brand name, all-lowercase titles, or ambiguous multi-category keywords
